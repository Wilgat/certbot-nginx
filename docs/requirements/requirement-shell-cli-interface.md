...
This requirement is the **project Single Source of Truth** for the **POSIX shell CLI interface** of the certbot-nginx tool: command surface, privilege typing, global flags, dispatcher behavior, output modes, and interactive vs non-interactive rules.
...
Ship-unit injection anchors (comments in `./certbot-nginx`): `DOMAIN_HELP_ROWS`, `DOMAIN_ABOUT_FIELDS`, `DOMAIN_DISPATCH_FLAGS`, `DOMAIN_DISPATCH_COMMANDS`, `DOMAIN_DISPATCH_ROUTES`.
...
| **Identity** | Retarget `APP_NAME`, descriptions, product `VERSION` on B | Leave `APP_NAME=certbot-nginx` on B ship unit |
...
|------|------------------------|
| **Product / binary name** | `certbot-nginx` (`APP_NAME`, default `certbot-nginx`) |
...
| **Dedicated system user** | Not for Type 0 CLI lifecycle; domain uses **nginx-adm** (domain SSOT) |
| **Domain catalog ownership** | `requirement-domain-certbot-nginx` |

#### Supported commands (normative for this project)

| Command | Type | Handler (current) | Required behavior |
|---------|------|-------------------|-------------------|
| *(no args — empty argv)* | Type 0 | install-ensure in `main_certbot_nginx_app` | **Type O** only; never domain host setup |
...
| `help` | Type 0 | `show_certbot_nginx_help` | Full usage; no foreign product DNA |
| `domains` / `email` / `nginx-conf` | Domain | domain handlers | See domain SSOT |

#### Global flags (normative wiring for this project)

| Flag | Required wiring |
|------|-----------------|
| `--quiet`, `-q` | Set `QUIET=1` in dispatcher |
| `--json` | Set `JSON=1` and `QUIET=1` |
| `--force` / `--reinstall` / `--reset` | Set `FORCE_REINSTALL=1` |
| `--no-cloudflare` | Set `USE_CLOUDFLARE=0` (domain) |

#### Dispatcher acceptance criteria (this project)

1. Unknown token after flag parse → fail loud with pointer to `certbot-nginx help`.  
2. Zero-arg → Type O install-ensure only (not help; not domain setup).  
3. Command routing **must** include every row in the command table above.  
4. Help text **must** stay aligned with that table (no orphan / unrouted listed commands).  
5. User-facing strings prefer centralized output helpers (not ad-hoc dual JSON families).
...
This requirement is satisfied for the certbot-nginx shell CLI when all of the following hold:
