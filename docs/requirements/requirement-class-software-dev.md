...
**Status**: Active (Version 1.0.0 – certbot-nginx class law + residual stack)  
**Area**: class  
...
| Field | Value (certbot-nginx) |
|-------|---------------------|
| **Project display name** | `certbot-nginx` (product root `README.md` H1 SSOT) |
...
| **Test runner** | `tests/run.sh` + `tests/test_cli.sh` + `tests/test_install_lifecycle.sh` + `tests/test_domain_surface.sh` |
| **Linter/formatter** | none as project law (shellcheck optional for maintainers, not required gate) |
| **Primary runtime / OS family** | POSIX Linux (and compatible UNIX where `/bin/sh` + coreutils/`sha256sum`/`mktemp` exist); end-user also needs nginx/certbot packages via domain setup |
...
| Bootstrap lineage | `requirement-bootstrap-chain` | Do not duplicate |
| Nginx/Certbot domain (four pillars) | `requirement-domain-certbot-nginx` | Do not duplicate |
...
| Date | Status | Note |
|------|--------|------|
| 2026-07-19 | Active | Specialized class law for certbot-nginx (review fix F1) |
