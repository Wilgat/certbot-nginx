...
This requirement is the **project Single Source of Truth** for **modular function organization** of the certbot-nginx POSIX shell CLI.
...
Optional multi-file layout under `src/` for future authoring **MAY** exist only if a build or pack step still produces **one** installable artifact and this requirement is updated. Until then, `./certbot-nginx` remains the single shipped script.
...
Optional `ALIGNMENT` / `See` / “fully synchronized with” lines in **product source** (`./certbot-nginx`) **MUST** cite only **live** `docs/requirements/requirement-*.md` paths that exist on disk and appear in `docs/requirements/index.md`.
...
When adding a function to `./certbot-nginx`:
...
A modular-structure change for certbot-nginx is **not done** if any of the following fail:
...
| `./certbot-nginx` | Implementation under modular design rules |
