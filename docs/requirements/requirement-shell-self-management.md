...
This requirement is the **project Single Source of Truth** for **CLI self-management** of the certbot-nginx POSIX shell tool: inspecting, upgrading, and removing its own installed binary (and related install artifacts) safely—especially for tools installed via one-command online install (`curl | sh`)—without requiring a separate package-manager workflow for routine updates.
...
| **Dispatcher** | `main_certbot_nginx_app` routes `version-check` → `version_check`; `self-update` → `self_update_v2`; `self-uninstall` → `self_uninstall`; `about` → `show_nginx_system_diagnostics` |
| **Install orchestrator SSOT** | `perform_self_install_v2` / `maybe_install_v2` (download + optional checksum + atomic install) |
| **Version compare** | string equality for already-latest; remote `VERSION` from `SCRIPT_URL` (strengthen to semver `ver_gt` when aligning further with bootstrap) |
...
Work claiming self-management support for certbot-nginx is **not done** if any of the following fail:
