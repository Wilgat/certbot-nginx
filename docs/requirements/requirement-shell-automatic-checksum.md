...
**Status**: Active (Version 1.0.1 – Shape A explicit)  
**Area**: shell  
...
This requirement is the **project Single Source of Truth** for **automatic companion-digest integrity** of the certbot-nginx POSIX shell tool: downloading a SHA-256 sidecar next to the install channel, verifying install/self-update downloads, and reporting the process **transparently** (companion **link**, expected **value**, and verification **result**).
...
**Must not confuse with:** Embedding a hash of `./certbot-nginx` *inside* `./certbot-nginx`; requiring operators to set `CHECKSUM` for every install; claiming independent host authenticity from same-channel SHA-256 alone.
...
| **In-repo publisher SSOT** | Companion file **`certbot-nginx.sha256`** (bare SHA-256 hex of `./certbot-nginx`) **MUST** ship next to the installable script for the release channel. |
| **Regenerate on change** | After every edit to `./certbot-nginx` that is published, regenerate `certbot-nginx.sha256` so automatic mode can match. |
...
1. Hash of `./certbot-nginx` stored **inside** `./certbot-nginx` as the verify target.  
...
1. Product root **`README.md` MUST** explain **automatic** checksum as the default: algorithm (SHA-256), companion URL pattern, in-repo `certbot-nginx.sha256`, match / mismatch / missing outcomes.  
...
|------|------------------------|
| **Specialization shape** | **Shape A** (default): automatic companion digest only as primary UX; digests held on the install path; **no** public process-env pin required for normal install |
...
2. Embed the expected SHA-256 of `./certbot-nginx` **inside** `./certbot-nginx` as verification.  
...
5. Document same-origin `CHECKSUM=$(curl …/certbot-nginx.sha256)` as higher assurance than automatic companion verification.  
...
Integrity work for certbot-nginx is **not done** if any of the following fail:
...
| `./certbot-nginx` | Ship unit implementation |
| `certbot-nginx.sha256` | In-repo companion digest |
